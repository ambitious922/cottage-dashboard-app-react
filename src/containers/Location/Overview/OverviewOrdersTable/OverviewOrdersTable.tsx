...
import { useGetLocationOverviewOrders } from 'api/query-hooks/order';
import { AppContext } from 'App';
import { TABLE_PAGINATION_SIZE } from 'constants/index';
import { useContext, useState } from 'react';
import { GraphQLResult } from '@aws-amplify/api';

import OverviewOrdersTableHeader from '../OverviewOrdersTableHeader';
...
const startOfTodayMillis = DateTime.now().startOf('day').toMillis();
const endOfTodayMillis = DateTime.now().endOf('day').toMillis();

const OverviewOrdersTable: React.FC<OrdersTableProps> = () => {
  ...
  const filters: FilterOrdersInput = {
    dateRange: {
      start: startOfTodayMillis.toString(),
      end: endOfTodayMillis.toString(),
    },
    statuses: [OrderStatus.PURCHASED],
  };
  const pagination: PaginationInput = {
    first: TABLE_PAGINATION_SIZE,
    after,
  };
  const locationOrdersQuery = useGetLocationOverviewOrders(locationInput, filters, pagination, {
    onSettled: (data) => {
  ...
  return (
    <div className="px-4 mt-4 bg-white border-b border-gray-200 rounded-lg shadow">
      <OverviewOrdersTableHeader totalOrders={orderList.length} />
...
export default OverviewOrdersTable;
